...
def collect_data(link: str) -> dict:
    ...
    if 'real estate project' in is_project:
        ...
        if req.status_code == 200:
            ...
            av_items_list = [i.strip().replace('"', '').rstrip(',') for i in av_items_list]
            ...
            for item in av_items_list:
                ...
                if pair[0] in ['id', 'zip_code', 'subtype', 'price', 'nb_bedrooms', 'kitchen_type',\
                               'land_surface', 'building_state','province', 'year_of_construction', 'geolocation']:
            ...
            appartments = ('apartment', 'ground-floor', 'penthouse', 'flat-studio','apartment-block', 'duplex', 'ground_floor')
            ...
            raw_property_info['p_type'] = 'house' if subtype in houses else 'apartment' if subtype in appartments else 'other'

            # living_area, furnished and open_fire
            raw_property_info['living_area'] = details_dict.get('Living area')
            raw_property_info['furnished'] = 1 if details_dict.get('Furnished') == 'Yes' else 0
            raw_property_info['open_fire'] = 1 if details_dict.get('How many fireplaces?') != None else 0
            ...
            raw_property_info['plot_surface'] = plot_surface if plot_surface not in [None, "''"]  else 0
            ...
            raw_property_info['swim_pool'] = swim_pool if swim_pool not in ['No', None] else 0
...
def concurrent_scraper(links_to_check):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        future_to_link = {executor.submit(collect_data, link): link for link in links_to_check}
